...
/** First http(s) URL in post text (what platforms build link cards from). */
export function firstURL(body: string): string | undefined {
  return body.match(/https?:\/\/[^\s)]+/)?.[0];
}

/** Posts (drafts and published) in a workspace. The list omits variants. */
...
/** One post WITH its variants (the list endpoint omits them). */
